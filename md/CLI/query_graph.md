...
Canonical variables để copy nhanh:
...
Nếu run của bạn có tên khác, chỉ cần thay `WORKDIR`. Với output cũ như `fresh_v2` hoặc `v3` chưa có manifest, giữ lại `WORKDIR` cũ nhưng nhớ truyền đủ HF embedding args như các lệnh mẫu bên dưới.
...
Nếu bạn đang chạy local `llama-server` turboquant ở `$BASE_URL`, dùng lệnh này để query ổn định:
...
### 3.1 Profile 7B canonical hiện tại: p2/c131072
...
Với flow canonical hiện tại, log server bạn cần kỳ vọng là:

- `n_ctx = 131072`
- `n_ctx_seq = 65536`
- `n_slots = 2`

Tức là mỗi slot query thực tế có khoảng `65536` context. Đây là lý do docs canonical hiện tại bám `p2`, không còn dùng `p4` ở phần hướng dẫn chính.

Khi query theo cấu hình này, client nên dùng `--llm_max_async 2` cho batch hoặc nhiều request song song. Với single-query, bạn có thể hạ về `1`.
...
- `--llm_max_async`: nên khớp `--parallel` của server nếu bạn chạy batch; với flow canonical hiện tại là `2`. Nếu chỉ hỏi từng câu, để `1` là đủ.
...
Với flow canonical hiện tại, dùng `--llm_max_async 2` cho batch và `1` cho single-query là hợp lý.

### 8.4 `--model`

Phải khớp alias model đang chạy trong server.

Ví dụ 7B canonical hiện tại:
