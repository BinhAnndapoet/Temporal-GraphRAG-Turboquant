...
```bash
bash scripts/run_7b_build_stack.sh \
  --output-dir outputs/build_graph/BUILD_qwen25_7b_p2_c131072_hf_nomic_cuda_384docs_v4 \
  --num-docs 384
```
...
## TL;DR — Canonical server command (khuyến nghị cho build/query/demo hiện tại)
...
`qwen25-7b-q8-ctkq8-ctvturbo3-c131072-p2-np3072`
...
```text
SERVER_qwen25_7b_p2_c131072_np3072
BUILD_qwen25_7b_p2_c131072_np3072_hf_nomic_384docs
```
...
|---|---|---|---|---|
| Test nhanh, local TurboQuant 7B | Qwen2.5 7B Q8 | `qwen2.5-7b-instruct-q8_0-00001-of-00003.gguf` | `p2/c131072/np3072` | Đây là profile canonical hiện tại: slot context khoảng 65K khi `--parallel 2`, khớp build/query/demo và build manifest |
...
## 2. Quick Start 7B: Qwen2.5 7B Q8, p2/c131072/np3072

Dùng profile này cho flow canonical hiện tại: build/query/demo trên `main`, output có `build_manifest.json`, HF-built graph, và local `llama-server` qua OpenAI-compatible API.

Tạo tmux:

```bash
tmux new -s srv_tq_turbo_7b_p2c131072_hf_nomic
```
...
./build/bin/llama-server \
  ...
  --parallel 2 \
  --n-predict 3072 \
  --log-file /home/guest/Projects/Research/Temporal-GraphRAG-Turboquant/logs/llama_server/cmp_tq_turbo_7b_p2c131072_hf_nomic_$(date +%Y%m%d_%H%M%S).log
```
...
- `-c 131072 --parallel 2`: tổng context 131K, chia 2 slot, mỗi slot khoảng 65K; đây là cấu hình build/query canonical hiện tại.
...
```bash
tmux attach -t srv_tq_turbo_7b_p2c131072_hf_nomic
```
...
```bash
tmux kill-session -t srv_tq_turbo_7b_p2c131072_hf_nomic
```
